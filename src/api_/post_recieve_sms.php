...
if ($conn->query($sql) === TRUE) {
    ...
    if (trim(strtolower($text_array[1])) == "bvh") {
        ...
        if($result->num_rows > 0) {
            while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
                $message .= $mons[$row["transmonth"]]. ": ".$row["totallBvPoints"]." pt(s)".PHP_EOL;
                //echo var_dump($row);
            }
            echo $message;
            sendSMS($gateway,$recipients, $message, $shortCode, $bulkSMSMode, $options);
    ...
    if (trim(strtolower($text_array[1])) == "price") {
        ...
        if($result->num_rows > 0) {
            while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {//$row[""]
                //$total_sales = $row[0];
                $message .= $row["ProductName"]." - ". " Price: ".$row["DPriceKSH"]."(DP), ".$row["CPriceKSH"]."(CP), BVs: ".$row["BVPoints"].PHP_EOL;
                //echo var_dump($row);
            }
            echo $result_text;
            //To confirm price
    ...
    if (trim(strtolower($text_array[1])) == "downline") {
        ...
        if($result->num_rows > 0) { //fetch_array(MYSQLI_NUM)
            while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
                //echo $row["fullNames"];
                $message .= $row["fullNames"]." ". $row["directUpline"].PHP_EOL;
    ...
    if (trim(strtolower($text_array[1])) == "bonus") {
        $MESSAGE = "";
        $sql = "SELECT amountPaid,DRN,MONTH(DateAwarded) AS monthAwarded, CASE WHEN status = 1 THEN '(Pd)' ELSE '(Os)' END AS PaidStatus FROM bonus WHERE DRN LIKE '%$text_array[2]%' ORDER BY DateAwarded DESC LIMIT 3";
        //To confirm bonus
        ...
        if($result->num_rows > 0) {
            while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
                $message .= $mons[$row["monthAwarded"]]. ": ".$row["amountPaid"]." ".$row["status"].PHP_EOL;
            }
            echo $message;
            sendSMS($gateway,$recipients, $message, $shortCode, $bulkSMSMode, $options);
        }
